...
public class CacheUtils {
     ...
     * @return
     */
    public static boolean setNx(String key,int time) {
        //自定义脚本
        DefaultRedisScript<Long> script = new DefaultRedisScript<>(SETNX_SCRIPT, Long.class);
        //执行脚本,传入参数,由于value没啥用,这里随便写死的"1"
        Long rst = redisTemplate.execute(script, Collections.singletonList(key), "1");
        //返回1,表示设置成功,拿到锁
        if(rst == 1L){
            log.debug("{}成功拿到锁", key);
            //设置过期时间
            expire(key,time);
            log.debug("{}已成功设置过期时间:{}秒", key, time);
            return true;
        }else{
            long expire = getExpire(key);
            log.debug("{}未拿到到锁,还有{}释放", key, expire);
            return false;
        }
    }
